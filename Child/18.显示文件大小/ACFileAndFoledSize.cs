...
namespace ACTool
{
    public class ACFileAndFoledSize : EditorWindow
    {
        ...
        {
            GetWindow(typeof(ACFileAndFoledSize), false, "文件操作面板").Show();
        }

        public static void OnShow()
        {
            if (GUILayout.Button("关闭文件大小", GUILayout.Width(200)))
            {
                number = 0;
                if (isShowSize == false) return;
                ClosePlaySize(false);
            }
            if (GUILayout.Button("显示文件大小", GUILayout.Width(200)))
            {
                number = 0;
        ...
        public static void OpenAndClosePlaySize()
        {
        ...
        public static void OpenPlaySize(bool isShow)
        {
            isShowSize = isShow;
            EditorPrefs.SetBool(FILESIZE, isShowSize);
            GetPropjectDirs();
        ...
        public static void ClosePlaySize(bool isShow)
        {
            isShowSize = isShow;
            EditorPrefs.SetBool(FILESIZE, isShowSize);
            GetPropjectDirs();
        ...
        private static void GetPropjectDirs()
        {
            ...
            {
                if (EditorUtility.DisplayCancelableProgressBar("进度", $"当前进度:{number}/{DirList.Count}", (float)number / DirList.Count))
                {
